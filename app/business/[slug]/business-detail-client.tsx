...
import { Phone, Mail, MapPin, MessageCircle, Building2, ShieldCheck, Star, Clock, Globe, Share2, Bookmark, CheckCircle2, MessageSquare } from 'lucide-react'
...
export default function BusinessDetailClient({ slug }: { slug: string }) {
  ...
  const handleAddReview = (newReview: { userName: string; rating: number; date: string; comment: string }) => {
    if (biz) {
      setBiz({
        ...biz,
        reviewCount: biz.reviewCount + 1,
        reviews: [{ id: Date.now().toString(), ...newReview }, ...biz.reviews]
      })
    }
  }
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-4 text-xs text-slate-300 flex-wrap">
                  ...
                  <span className="font-bold text-amber-400 flex items-center gap-1">
                    ...
                    {biz.rating} ({biz.reviewCount} customer reviews)
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-xs space-y-6">
            <div className="flex justify-between items-center pb-4 border-b border-slate-100">
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className="text-xs text-slate-500 font-medium">({biz.reviewCount} total reviews)</span>
                </div>
              </div>

              <button
                onClick={() => setShowReviewModal(true)}
                className="px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-xl transition-colors inline-flex items-center gap-1.5 cursor-pointer"
              >
            ...
            {biz.reviews.length === 0 ? (
              ...
              <div className="space-y-4">
                {biz.reviews.map((rev) => (
                  <div key={rev.id} className="p-4 rounded-xl bg-slate-50 border border-slate-200/60 space-y-2">
